// api/translator.js
// Функции для работы с LibreTranslate API
const API_URL = "https://translate.argosopentech.com/translate";
const DETECT_URL = "https://translate.argosopentech.com/detect";
const LANGUAGES_URL = "https://translate.argosopentech.com/languages";

async function translateText(text, sourceLang = 'cs', targetLang = 'ru') {
  if (!text || text.trim() === '') {
    return '';
  }
...
const App = () => {
  ...
  // Функция для получения перевода слова через LibreTranslate API
  const fetchTranslation = async (word) => {
    ...
    try {
      ...
      // Возвращаем объект с исходным словом, но без перевода
      return {
        word: word,
        translations: [],
        samples: [],
        note: "Не удалось получить перевод."
      };
    }
  };
